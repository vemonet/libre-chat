...
@cli.command("start")
def start(
    model: str = typer.Option(defaults.model_path, help="Path to the model binary"),
    vector: str = typer.Option(defaults.vector_db_path, help="Path to the vector db folder"),
...
def vuild(
    model: str = typer.Option(defaults.model_path, help="Path to the model binary"),
    vector: str = typer.Option(defaults.vector_db_path, help="Path to the vector db folder"),
    data: str = typer.Option(defaults.data_path, help="Path to the data folder to vectorize"),
    debug: bool = typer.Option(True, help="Display debug logs"),
) -> None:
